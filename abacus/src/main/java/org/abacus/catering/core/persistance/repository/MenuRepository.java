...
public interface MenuRepository extends CrudRepository<CatMenuEntity,Long>{
	...
	@Query("select a from CatMenuEntity a inner join fetch a.menuInfo i where a.fiscalYear.id = :fiscalYearId and a.menuDate <= :p_menuDate order by a.menuDate, a.id")
