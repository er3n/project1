...
public interface DefReferenceRepository extends CrudRepository<DefReferenceEntity, Long> {
	...
	@Query("select a from DefReferenceEntity a where a.organization.id = :organizationId and a.type.id = :typeId")
	List<DefReferenceEntity> getReferenceList(@Param("organizationId")String organizationId, @Param("typeId") String typeId);

	@Query("select a from DefReferenceEntity a where a.organization.id = :organizationId and a.type.id = :typeId and a.refType = :itemEnum")
