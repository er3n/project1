...
public interface DefTaskRepository extends CrudRepository<DefTaskEntity, Long> {
	...
	List<DefTaskEntity> getTaskListRepo(@Param("organizationId")String organizationId, @Param("typeId") String typeId);

	@Query("select a from DefTaskEntity a where a.organization.id = :organizationId and a.type.id = :typeId order by a.code")
	DefTaskEntity getTaskRepo(@Param("organizationId")String organizationId, @Param("typeId") String typeId);
