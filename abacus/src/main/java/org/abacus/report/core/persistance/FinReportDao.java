...
public class FinReportDao implements Serializable {
	...
	public List<FinDetailEntity> getFinState(ReportSearchCriteria reportSearchCriteria) {
		...
		sb.append("select {organization.*}, {fiscalPeriod2.*}, {item.*}, v.baseDetailAmount ");
		sb.append("  from org_organization organization, org_fiscal_period fiscalPeriod2, def_item item,");
		sb.append("     (select c.organization_id, c.fiscal_period2_id, d.item_id, sum(d.base_detail_amount*d.tr_state_detail) baseDetailAmount ");
		...
		sb.append("       group by c.organization_id, c.fiscal_period2_id, d.item_id) v");
