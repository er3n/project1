...
public interface ReqDetailRepository extends CrudRepository<ReqDetailEntity, Long>, TraDetailRepository<ReqDetailEntity> {
	...
	@Query("select d from ReqDetailEntity d inner join fetch d.item i inner join fetch d.department left outer join fetch d.itemUnit u left outer join fetch d.departmentOpp left outer join fetch i.itemUnitSet left outer join fetch d.offerSet os where d.document.id = :documentId and d.refDetailId is null")
