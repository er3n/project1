...
public interface StkDetailRepository extends CrudRepository<StkDetailEntity, Long>, TraDetailRepository<StkDetailEntity> {
	...
	@Query("select distinct(d) from StkDetailEntity d inner join fetch d.item i inner join fetch d.department left outer join fetch d.itemUnit u left outer join fetch d.departmentOpp left outer join fetch i.itemUnitSet where d.document.id = :documentId and d.refStkDetailId is null")
