...
public interface StkDetailTrackRepository extends CrudRepository<StkDetailTrackEntity, Long>{
	...
	@Query("select t from StkDetailTrackEntity t inner join t.detail d where d.fiscalYear.id = :fiscalYearId and d.item.id = :itemId and d.department.id = :departmentId and d.trStateDetail = 1 and t.baseTrackCount > t.baseUsedCount order by t.dueTrackDate")
