...
public interface UserRepository extends CrudRepository<SecUserEntity, String> {
	...
	@Query("select distinct(a.id) from SecGroupMemberEntity ur, SecGroupAuthorityEntity ra, SecAuthorityEntity a where ur.user.id = :id and ur.group.id = ra.id and ra.authority.id = a.id")
