CREATE OR REPLACE VIEW v_fin AS SELECT doc.organization_id, doc.fiscal_period2_id as period_id, doc.doc_date, dtl.glc_id, itm.type_id AS type_code, typ.name type_name, itm.code AS item_code, itm.name AS item_name, (CASE WHEN dtl.tr_state_detail=1 THEN ' Borç' ELSE ' Alacak' END) as tr_state, (tr_state_detail*dtl.base_detail_amount) as gl_amount, (CASE WHEN dtl.tr_state_detail=+1 THEN dtl.base_detail_amount ELSE 0 END) debit_amount, (CASE WHEN dtl.tr_state_detail=-1 THEN dtl.base_detail_amount ELSE 0 END) credit_amount FROM tra_detail dtl, fin_document doc, def_item itm, def_type typ WHERE doc.id = dtl.document_fin_id AND itm.id = dtl.item_id and typ.id = itm.type_id;
...
create or replace view v_stk as select doc.fiscal_period1_id, doc.fiscal_period2_id, doc.doc_date, dtl.item_id, itm.code item_code, itm.name item_name, dtl.department_id, dpr.code department_code, dpr.name department_name, dtl.base_detail_count, dtl.tr_state_detail from tra_detail dtl, stk_document doc, def_item itm, org_department dpr where doc.id = dtl.document_stk_id and itm.id = dtl.item_id and dpr.id = dtl.department_id and itm.type_id = 'ITM_SR_ST';

create or replace view v_bud as select doc.fiscal_year_id, det.fiscal_period_id, det.budget_rx, det.budget_type, det.accrue_date, det.budget_amount* (case when det.budget_rx = 'BUD_R' then 1 ELSE (-1) end) budget_amount from bud_detail det, bud_document doc where det.document_id = doc.id;
