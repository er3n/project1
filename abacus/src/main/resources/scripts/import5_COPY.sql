...
insert into def_item_value(id, version, item_id, value_type_enum, value_id) select nextval('seq_id'), 0, id, 'VAL_RECEIPT', (select v.id from def_value v where v.code='T0001' and organization_id='00') from def_item where organization_id='00' and type_id ='ITM_SR_ST' and class_enum='STK_P';
...
insert into sec_user_organization (id, user_id, organization_id, version) select nextval('seq_id'), 'admin', replace(organization_id,'00','01'), 0 from sec_user_organization where  organization_id like '#%' and user_id = 'admin';
